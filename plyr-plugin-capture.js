(function (document) {
	...
	function capture (player, label) {
		let width = player.media.videoWidth;
		let height = player.media.videoHeight;
	...
	document.addEventListener('ready', event => {
		...
		if(Array.isArray(config.controls)&&config.controls.includes('capture')){
			const capture_label = config.i18n.capture || 'Capture';
			...
			let btnElement = document.querySelector('button[data-plyr="capture"]');
			btnElement.addEventListener('click', () => {
				capture(curPlayer, capture_label);
			});
		}
	});
})(document);